...
function drawCicle(x,y,r,color,blur='white'){
    ctx.beginPath()
...
function drawSnowman(){
    drawCicle(canvas.width/2,canvas.height-103,100,'white')
    drawCicle(canvas.width/2,canvas.height-250,60,'white')
    drawCicle(canvas.width/2,canvas.height-340,40,'white')
    drawCicle(canvas.width/2-15,canvas.height-350,5,'black')
    drawCicle(canvas.width/2+15,canvas.height-350,5,'black')
...
function draw(){
    ...
    for(let i = 0; i<snowX.length;i++){
        drawCicle(snowX[i],snowY[i],snowR[i],snowColor[i],snowBlur[i])
        snowY[i]++
